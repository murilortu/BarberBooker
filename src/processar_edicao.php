...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute() === TRUE) {
        echo '<div class="alert alert-success" role="alert">Edição realizado com sucesso!</div>';
        header("refresh:2;url=listar_agendamentos.php"); // Redirecionar para index.php após 2 segundos
    } else {
        echo '<div class="alert alert-danger" role="alert">Erro ao realizar agendamento: ' . $stmt->error . '</div>';
        header("refresh:5;url=listar_agendamnetos.php"); // Redirecionar para index.php após 5 segundos
    }
